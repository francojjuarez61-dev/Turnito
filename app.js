...
// UI only: toggle del desenfoque del fondo (para ver la foto nítida)
const STORAGE_KEY_APP_BG_BLUR = 'bt_vip_appbg_blur_v1'; // '1' = con blur, '0' = sin blur
...
function applyAppBgBlur(){
  try{
    let v = localStorage.getItem(STORAGE_KEY_APP_BG_BLUR);
    if (v === null){
      v = '1';
      localStorage.setItem(STORAGE_KEY_APP_BG_BLUR, v);
    }

    const on = v !== '0';
    document.body.classList.toggle('appbg-blur-off', !on);
    // no hace falta clase "on" porque el default ya tiene blur

    const btn = document.getElementById('appBgBlurToggle');
    if (btn){
...
const appBgBlurToggle = document.getElementById('appBgBlurToggle');
...
// UI only: toggle blur ON/OFF del fondo
appBgBlurToggle?.addEventListener('click', () => {
  feedback('tap');
  try{
    const cur = localStorage.getItem(STORAGE_KEY_APP_BG_BLUR);
    const next = (cur === '0') ? '1' : '0';
    localStorage.setItem(STORAGE_KEY_APP_BG_BLUR, next);
  }catch(_){ }
  applyAppBgBlur();
});
