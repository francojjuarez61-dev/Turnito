...
function loadConfig(){
  try{
    ...
    if (Array.isArray(parsed.limits) && parsed.limits.length===2){
      const a = parsed.limits[0] || {};
      const b = parsed.limits[1] || {};
      const l0 = { hour: Number(a.hour), min: Number(a.min) };
      const l1 = { hour: Number(b.hour), min: Number(b.min) };
      if (Number.isFinite(l0.hour) && Number.isFinite(l0.min) && Number.isFinite(l1.hour) && Number.isFinite(l1.min)){
        cfg.limits = normalizeLimits(l0, l1);
      }
    }
    return cfg;
  }catch(_){
...
function closeMenu(){
  if (!menuPopover || !menuBtn) return;
  menuPopover.classList.add('is-hidden');
  menuBtn.setAttribute('aria-expanded', 'false');
}

function toggleMenu(){
  if (!menuPopover || !menuBtn) return;
  const willOpen = menuPopover.classList.contains('is-hidden');
  if (willOpen) {
    feedback('soft');
    menuPopover.classList.remove('is-hidden');
...
document.addEventListener('click', (e) => {
  if (!menuPopover || !menuBtn) return;
  if (menuPopover.classList.contains('is-hidden')) return;
  const inside = e.target.closest('#menuPopover') || e.target.closest('#menuBtn');
  if (!inside) closeMenu();
});
...
const set_limit_day = document.getElementById('set_limit_day');
const set_limit_night = document.getElementById('set_limit_night');
...
function fillSettingsForm(){
  ...
  // horarios límite
  if (Array.isArray(config.limits) && config.limits.length===2){
    set_limit_day.value = `${String(config.limits[0].hour).padStart(2,'0')}:${String(config.limits[0].min).padStart(2,'0')}`;
    set_limit_night.value = `${String(config.limits[1].hour).padStart(2,'0')}:${String(config.limits[1].min).padStart(2,'0')}`;
  }

}

function parseTimeInput(input, fallback){
  const raw = (input?.value || '').trim();
  const m = /^([01]?\d|2[0-3]):([0-5]\d)$/.exec(raw);
  if (!m) return fallback;
  return { hour: Number(m[1]), min: Number(m[2]) };
}

function normalizeLimits(l1, l2){
  const a = { hour: l1.hour, min: l1.min };
  const b = { hour: l2.hour, min: l2.min };
  const ma = a.hour*60 + a.min;
  const mb = b.hour*60 + b.min;
  const first = (ma<=mb) ? a : b;
  const second = (ma<=mb) ? b : a;
  const label = (t)=> `${String(t.hour).padStart(2,'0')}:${String(t.min).padStart(2,'0')}`;
  return [
    { label: label(first), hour: first.hour, min: first.min },
    { label: label(second), hour: second.hour, min: second.min }
  ];
}
...
function applyConfigFromForm(){
  ...
  // horarios límite (2 turnos)
  const currentL0 = cfg.limits?.[0] || { hour: 13, min: 0 };
  const currentL1 = cfg.limits?.[1] || { hour: 22, min: 0 };
  const tDay = parseTimeInput(set_limit_day, { hour: currentL0.hour, min: currentL0.min });
  const tNight = parseTimeInput(set_limit_night, { hour: currentL1.hour, min: currentL1.min });
  cfg.limits = normalizeLimits(tDay, tNight);
